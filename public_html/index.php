...
<html>
	...
	<head>
		...
		<div id="box1" style="<?php $conn = mysqli_connect('localhost','root','root','varaukset'); $query="SELECT varaus FROM nyt WHERE nimi='tettapaikka1'"; $result = mysqli_query($conn,$query); $row = mysqli_fetch_array($result); if ($result==1) {echo 'background-color: blue;';} if ($result==2) {echo 'background-color: red;';} ?>"> test</div>
		</div> 
